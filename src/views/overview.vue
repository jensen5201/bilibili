  ...
  div
    ...
    ul
      li(v-for="item, key, idx of userInfo")
        span {{`${idx+1}、${key}:${item}`}}
    div(v-loading='preLoading' id="main" style="width: 600px;height:400px;")
    div(v-loading='preLoading' id="second" style="width: 600px;height:400px;")
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    drawSecondEchart() {
      ...
      // 使用刚指定的配置项和数据显示图表
      let option = this.setSecondOption();
      myChart.setOption(option);
    }
  },
  async created() {
